...
public abstract class Humanoid implements Controllable, PhysicsObject, Entity {
    ...
    // movement
    protected static final float WALK_ACCEL = 10.0f;
    private static final float MAX_WALK_SPEED = 30.0f;
    protected static final float JUMP_FORCE = 50;

    // box2d physics objects
    protected Body body;
    private Vector2 startPosition;
    protected final Vector2 impulse = new Vector2(); // reusable, just clear both dimensions before using
    ...
    // jumping state
    protected static final float EXTENDED_JUMP_SECONDS = 0.2f;
    protected boolean jumping = false;
    protected float extendedJumpTimer;
    protected float walkCooldown; // used to prevent sticking to the wall when climbing

    public Humanoid(Vector2 startPosition, String sprite) {
    ...
    public void update(float delta) {
        ...
        if (jumping) {
            if (extendedJumpTimer > 0) {
                impulse.set(body.getLinearVelocity().x, JUMP_FORCE);
